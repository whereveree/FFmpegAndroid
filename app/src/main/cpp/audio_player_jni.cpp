...
#define tag "AudioPlayerJNI"
#define SLEEP_TIME (16000)

JNI_FUN_AUDIO(initNativePlayer, jlong) {
    auto *audioPlayer = new FFAudioPlayer();
    LOG_D(tag, "native_init: Created FFAudioPlayer instance: %p", audioPlayer);
...
AUDIO_PLAYER_FUNC(void, native_1play, long context, jstring path, jstring filter) {
    ...
    jmethodID audio_track_method = env->GetMethodID(audio_class, "createAudioTrack", "(II)Landroid/media/AudioTrack;");
...
AUDIO_PLAYER_FUNC(void, native_1release, long handle) {
    auto *audioPlayer = reinterpret_cast<FFAudioPlayer *>(handle);
    if (audioPlayer) {
